...
// Inicializa o porteiro (Supabase)
...
export default function Admin() {
  ...
  // Busca dados apenas se estiver logado
  useEffect(() => {
    if (user) {
      fetch("https://motor-saas-ltb4.onrender.com/lojas").then(r => r.json()).then(setLojas).catch(() => {});
      // Busca apenas os produtos deste usuário logado
    ...
    try {
      const res = await fetch("https://motor-saas-ltb4.onrender.com/produtos", {
        ...
        body: JSON.stringify({ 
          ...
          foto_url: fotoUrl, 
          loja_id: lojaId,
          user_id: user.id // Vincula o produto ao usuário logado
        }),
      });
      if (res.ok) {
  ...
  // Enquanto verifica o login, mostra tela vazia
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        input, select { width: 100%; padding: 10px; border-radius: 6px; border: 1px solid var(--border); margin-top: 5px; outline: none; }
          ...
          {tab === "add" ? (
            <div className="card">
              <form onSubmit={cadastrarProduto} style={{ display: "grid", gap: "15px" }}>
                ...
                <div>
                  <label style={{ fontSize: "12px", fontWeight: 600 }}>URL da Imagem</label>
